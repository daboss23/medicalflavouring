...
cleared, so the bottles, their caps and their labels survive untouched.
    ...
    python3 tools/knockout-bg.py assets/in.png assets/out.png [tolerance]
"""
import sys
...
def knockout(src, dst, tolerance=26):
    ...
    im.save(dst)
    print(f"{src} -> {dst}: cleared {cleared:,} px, cropped to {im.size[0]}x{im.size[1]}")


if __name__ == "__main__":
    if len(sys.argv) < 3:
        sys.exit(__doc__)
    knockout(sys.argv[1], sys.argv[2], int(sys.argv[3]) if len(sys.argv) > 3 else 26)
